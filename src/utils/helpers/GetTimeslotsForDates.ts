 export const dates = slots.reduce((acc: { date: string; day: string }[], slot) => {
  ...
  export const getTimeSlotsForDate = (dateStr: string | null) => {
    if (!dateStr) return []
    return slots
      .filter(slot => {
        const startDate = new Date(slot.startDateTime)
      ...
      .map(slot => {
        ...
        return {
          ...
          slotId: slot.startDateTime  // ISO string, e.g., "2025-10-19T14:00:00.000Z"
        }
      })
  }
